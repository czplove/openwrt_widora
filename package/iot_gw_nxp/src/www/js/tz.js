var tzones = [
    ...
    ["America/Argentina/BuenosAires","ART3"],
    ...
    ["America/Argentina/RioGallegos","ART3"],
    ...
    ["America/NorthDakota/NewSalem","CST6CDT,M3.2.0,M11.1.0"],
...
function tzSelector( index ) {
    console.log( "Make TZ selector, current = '" + index + "'" );
    var html = "";
    html += "<select id='tz_sel'>";
    for ( i=0; i<tzones.length; i++ ) {
        console.log( "land " + tzones[i] + ", abbr = '" + tzones[i][1] + "'" );
        html += "<option value='" + tzones[i][1] + "'";
        if ( i == index ) {
            html += " selected";
        }
        html += ">" + tzones[i][0] + "</option>";
    }
    html += "</select>";
    return( html );
}

function tzFindIndexZonename( zonename ) {
    for ( i=0; i<tzones.length; i++ ) {
        if ( tzones[i][0] == zonename ) return i;
    }
    return 0;
}

function tzFindIndexTimezone( timezone ) {
    for ( i=0; i<tzones.length; i++ ) {
        if ( tzones[i][1] == timezone ) return i;
    }
    return 0;
}

function tzGetZonename( index ) {
    return tzones[index][0];
}
